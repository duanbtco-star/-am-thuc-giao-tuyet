...
Sau khi tạo xong domain/entity.go và domain/repository.go (Interface),
...
```
Template: .agent/templates/go_module_skeleton.md
Vị trí: internal/modules/{module_name}/
```

### 3.2 Tạo các file
1. `domain/entity.go` - Domain entities
2. `domain/repository.go` - Repository interface
3. `domain/service.go` - Business logic
4. `application/dto.go` - Request/Response DTOs
5. `application/usecase.go` - Use cases
6. `infrastructure/postgres_repo.go` - DB implementation
7. `infrastructure/http_handler.go` - HTTP handlers

### 3.3 Đăng ký routes
```go
// cmd/api/routes.go
router.Group("/{feature}").
    GET("", handler.List).
...
### 3.4 Chạy tests
```bash
go test ./internal/modules/{module_name}/... -v
```
...
## Step 4: Frontend UI (Next.js)

### 📍 CHECKPOINT: `frontend_complete`

### 4.1 Tạo components
```
Template: .agent/templates/next_component_skeleton.md
Vị trí: src/app/(dashboard)/{module}/{feature}/
```

### 4.2 Tạo các file
1. `page.tsx` - Main page
2. `components/{Feature}List.tsx` - List view (AG Grid)
3. `components/{Feature}Form.tsx` - Create/Edit form
4. `components/{Feature}Modal.tsx` - Modal wrapper
5. `hooks/use{Feature}.ts` - React Query hooks

### 4.3 Thêm translations
```
Vị trí: src/locales/vi/{module}.json
...
{ name: t('{feature}.title'), href: '/{module}/{feature}' }
```
